...
public interface PlantRepository
	...
	@Query("SELECT p FROM Plant p JOIN FETCH p.hydration JOIN FETCH p.sunlight WHERE LOWER(p.commonName) LIKE LOWER(CONCAT('%', :searchText, '%')) OR LOWER(p.latinName) LIKE LOWER(CONCAT('%', :searchText, '%'))")
	Collection<PlantItem> findPlantsBySearchText(@Param("searchText") String searchText);
